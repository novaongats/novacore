...
export function HomePage({ user }) {
  ...
  // Recent activity (all time, limited)
  ...
  const stats = useMemo(() => {
    ...
    const prevRevenue  = sumBy(prev,    e => e.amount);

    // Costs: salesCosts items + cashbook + rev-share
    let totalCost = 0;
    for (const c of costs) totalCost += sumBy(asArray(c.items), i => i.amount);
    totalCost += sumBy(expenses, e => e.amount);
    for (const cat of cats) {
      ...
      totalCost += Math.round(rev * (100 - pct) / 100);
    }
    ...
    <div>
      ...
      <div style=${{
        ...
        <${Kpi} label="当月売上"     value=${formatYen(stats.totalRevenue)}
               ...
               accent="primary" icon="▲" />
        <${Kpi} label="当月コスト"   value=${formatYen(stats.totalCost)}
               accent="danger"  icon="━" />
